...
<form action="{{ route('items.update', $item->id) }}" method="POST" class="mt-6 space-y-6">
    ...
    <div class="mb-3">
        ...
        <input type="text" name="price" class="mb-12 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full" value="{{ $item->price }}" required>
		
    </div> 
    <div class="mb-3">
        <label for="quantity" class="block font-medium text-sm text-gray-700">Default Quantity</label>
        <input type="text" name="quantity" class="mb-12 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full" value="{{ $item->default_quantity }}" required>
